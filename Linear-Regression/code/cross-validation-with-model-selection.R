...
HM <- read.csv("./dataset/haydn-mozart.csv")
HM[,1] <- NULL # first col is just name of quartet, remove it
dim(HM) # 285 observations, 1116 columns: more predictors than observations. (X-transpose X) not invertible.
# X not at full rank
...
# Split dataset to training and Validation set: 80-20
...
# Note: because n < p, we cannot do backward prediction since we cannot fit a model with all predictors
...
summary(m1) # we see here that the multiple R-squared is really high:
# either we're predicting really well
# or we are overfitting # so let's check with cross-validation
...
# if RMSE on validation is much larger than RMSE on train, we have overfitted the model

# Try ICM to search for a model with a potentially better BIC
# than the one found with stepwise
pen <- log(nrow(trainSet)) # BIC penalty
varlist = c()
...
# the comparison of the RMSEs show that we're overfitting with ICM using BIC
...
# K-fold cross validation to choose model selection method on the different pernalties
K <- 5
